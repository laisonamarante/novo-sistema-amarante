...
import { Input, Select, Btn, Card, Spinner, Alert, Badge } from '../../components/ui'
...
import { AbaDadosGerais } from './processo-form/abas/AbaDadosGerais'
import { AbaEtapas } from './processo-form/abas/AbaEtapas'
import { AbaDocumentacao } from './processo-form/abas/AbaDocumentacao'
        ...
        <div className="p-5">
          ...
          {aba==='dadosGerais' && (
            <AbaDadosGerais
              ...
              novaPendencia={novaPendencia}
              onChangeNovaPendencia={setNovaPendencia}
              onRegistrarPendencia={()=>{ if(novaPendencia.trim()) registrarPendencia.mutate({processoId:Number(id),descricao:novaPendencia.trim()}) }}
              registrandoPendencia={registrarPendencia.isPending}
            />
          )}
          ...
          {aba==='etapas' && (
            <AbaEtapas
              onAbrirConcluir={(target)=>{setConcluirEtapaTarget(target);setConcluirEtapaObs('');setProximoResponsavelEtapaId(0);setModalConcluirEtapa(true)}}
              onAbrirPendente={(target, obsAtual)=>{setPendenteEtapaTarget(target);setPendenteEtapaObs(obsAtual);setModalPendenteEtapa(true)}}
              onAbrirObs={(target, obsAtual)=>{setEtapaObsTarget(target);setEtapaObsTexto(obsAtual);setModalObsEtapa(true)}}
              onReabrirEtapa={(target, etapaNome)=>{ if (confirm(`Reabrir a etapa "${etapaNome}" e voltar o processo para ela?`)) reabrirEtapa.mutate(target) }}
            />
          )}
          ...
          {aba==='documentacao' && (
            <AbaDocumentacao
              docTiposPorFluxo={docTiposPorFluxo.data}
              onUploadFile={handleFileUpload}
              onAprovarDoc={(docId)=>aprovarDoc.mutate({id:docId})}
              onAbrirReprovarModal={(target)=>{setReprovarModal(target);setReprovarMotivo('')}}
              onAbrirPreview={abrirDocumento}
              onExcluirDoc={(docId)=>excluirDocumento.mutate({id:docId})}
            />
          )}
